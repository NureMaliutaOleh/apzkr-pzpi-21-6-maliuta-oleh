...
namespace SmartInlet.Server.Services.Email
{
	public class EmailService : IEmailService
	{
		...
		private string EmailTemplatesFolder =>
			Path.IsPathRooted(configuration.EmailTemplatesFolder) ?
		...
		public async Task SendEmailUseTemplateAsync(
			string email,
			string templateName,
			Dictionary<string, string>? parameters = null, string? subject = null)
		{
			...
			subject ??= Regex.Match(
				template,
				@"<meta name=""subject"" content=""(.*)""").Groups[1].Value;
